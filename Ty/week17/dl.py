...
class DialogueSystem:
    ...
    def load(self):
        self.all_node_info = {'重听':{'intent':['我没听清','再重复一遍','你说什么']}} #
    ...
    def intent_judge(self, memory):
        #意图识别，匹配当前可以访问的节点
        query = memory['query']
        max_score = self.calucate_node_score(query, '重听') #
        hit_node = '重听' #
    ...
    def run(self, query, memory):
        ...
        memory: 用户状态
        '''
        if not memory.get("available_nodes", []): #
            memory = {"available_nodes": self.init_node} #
        memory["query"] = query
...
if __name__ == '__main__':
    ds = DialogueSystem(["scenario-买衣服_node1","scenario-看电影_node1"])
